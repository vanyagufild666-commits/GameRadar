...
## Вариант 3: хостинг с автодеплоем из GitHub

Подходит для Railway, Render, Fly.io и панелей, которые собирают сервис прямо из репозитория.

Настройки сервиса:

- **Build**: `pip install -r requirements.txt`
- **Start**: `python bot.py`
- **Python**: 3.11 (использованная версия — 3.11.16, зафиксирована в `.python-version`)
- **Переменные**: `BOT_TOKEN` (обязательно), `DB_PATH=/data/bot.db`, при необходимости
  `ADMIN_IDS`, `BOT_USERNAME`, `CHANNEL_LINK`, `CONTACT_LINK`

Что важно учесть:

1. Подключите постоянный диск (persistent volume) и смонтируйте его туда, куда указывает
   `DB_PATH`. Без него база теряется при каждом деплое вместе со всеми анкетами.
2. Входящий порт боту не нужен: он работает на long polling. Если панель считает сервис
   живым только при открытом порте, задеплойте процесс как worker/background service.
3. `BOT_TOKEN` задавайте только в переменных окружения панели — не в Dockerfile, не в коде
   и не в команде запуска, иначе он попадёт в историю сборки.
4. Один и тот же токен нельзя запускать одновременно на двух хостах: второй экземпляр
   получит `Conflict: terminated by other getUpdates`. Останавливайте локального бота,
   когда работает облачный.
5. Обновление — обычный `git push` в `main`: панель пересоберёт и перезапустит сервис.
